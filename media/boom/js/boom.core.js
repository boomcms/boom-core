...
$.extend($.boom, {
	...
	/**
	Boom ORM. Ajax access to the Kohana ORM. Not implemented.
	@class
	*/
	orm : function(type, options, callback){
		...
		/**
		Make an ajax call to the Kohana ORM.
		*/
		this._find = function(callback){

			/*var data = 'type=' + this._type + '&' + $.param(options);

			$.ajax({
				url: $.boom.config.orm.getobjUrl,
				data: data,
				dataType: 'json',
				cache: false,
...
$.extend($.boom, {
	...
	/**
	Does something funky with timeouts. Not sure what.
	*/
...
$.extend($.boom.heartbeat.prototype, {
	/** @lends $.boom.heartbeat */

	/**
	Sets an interval. What for?
	*/
		_init: function(){

		if (this.options.interval) {
			this.options.timer = setInterval(this.options.action, this.options.interval);
		}
	}
