    ...
    <div class="b-asset-metric-form">
            <div class="form-left">
                <form action="/boomcms/asset-manager/metrics/<?= $asset->id ?>/details" method="post">
                    ...
                    <a href="/boomcms/asset-manager/metrics/<?= $asset->id ?>/details">CLEAR RANGE</a>
                </form>
            </div>
        ...
        <?php if (trim($request->get('from')) !== '' && trim($request->get('to')) !== '') { ?>
            <h3>Data from <?= $request->get('from') ?> to <?= $request->get('to') ?></h3>
        <?php } ?>
