	...
	<div class="b-template">
		...
		<select id='template' name='template_id'>
			<?php foreach ($templates as $t): ?>
				<option value='<?= $t->getId() ?>' data-description="<?= $t->getDescription() ?>" data-count='<?= $countPages(['tempalte' => $t]) ?>'<?php if ($template_id == $t->getId()): ?> selected='selected'<?php endif ?>><?= $t->getTheme() ?> - <?= $t->getName() ?></option>
			<?php endforeach ?>
		</select>
