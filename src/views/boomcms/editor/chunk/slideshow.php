    ...
    <section>
        ...
        <ol id="b-slideshow-editor-slides">
            <?php foreach ($chunk->getSlides() as $slide): ?>
                ...
                <li>
                    <label>
                        <input type="radio" value="<?= $slide->getId() ?>" name="slide" data-asset="<?= $slide->getAssetId() ?>" data-title="<?= $slide->getTitle() ?>" data-url="<?= $link->url() ?>" data-caption="<?= $slide->getCaption() ?>" data-linktext='<?= $slide->getLinkText() ?>' />
                        <img src="<?= $assetURL(['id' => $slide->getAssetId(), 'height' => 100]) ?>" />
                    </label>
                </li>
            <?php endforeach ?>
        </ol>
