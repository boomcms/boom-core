<html>
	<head>
		<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
		<meta name="viewport" content="width=device-width; initial-scale=1.0; maximum-scale=1.0;">
		<title></title>
		<style type="text/css" media="screen">

		</style>
	</head>
    <body style="-webkit-text-size-adjust:none !important;padding:0;">
		<table class="body" width="100%" cellspacing="0" cellpadding="0" style="margin: 0; padding: 0;background-color:#f6f6f6;">
			<tr>
				<td align="center" valign="top">
					<table class="maincontent" cellspacing="0" cellpadding="0" style="margin: 0 auto; padding: 0; width:600px;">
						<tr>
							<td width="600" height="30">
								<img src="<?= URL::site('/vendor/boomcms/boom-core/email_assets/spacer.gif', $request) ?>" />
							</td>
						</tr>
						<tr>
							<td>
								<table class="header" width="100%" cellspacing="0" cellpadding="0" style="margin: 0; padding: 0;background-color:#FFD200;color:#404040;">
									<tr>
										<td width="10" height="10">
											<img src="<?= URL::site('/vendor/boomcms/boom-core/email_assets/spacer.gif', $request) ?>" />
										</td>
										<td width="580" height="10">
											<img src="<?= URL::site('/vendor/boomcms/boom-core/email_assets/spacer.gif', $request) ?>" />
										</td>
										<td width="10" height="10">
											<img src="<?= URL::site('/vendor/boomcms/boom-core/email_assets/spacer.gif', $request) ?>" />
										</td>
									</tr>
									<tr>
										<td width="10" height="10">
											<img src="<?= URL::site('/vendor/boomcms/boom-core/email_assets/spacer.gif', $request) ?>" />
										</td>
										<td width="580" style="font-family: Arial, Helvetica, sans-serif !important; margin: 0; font-size:16px !important;text-align:center;">
											<img src="<?= URL::site('/vendor/boomcms/boom-core/email_assets/uxb_logo_sm.png', $request) ?>" style="vertical-align:middle;"/> Email from BoomCMS, by <a href="http://www.uxblondon.com/services/content-management" style="color:#404040; text-decoration:underline !important;font-weight:bold;">UXB London</a>.
										</td>
										<td width="10" height="10">
											<img src="<?= URL::site('/vendor/boomcms/boom-core/email_assets/spacer.gif', $request) ?>" />
										</td>
									</tr>
									<tr>
										<td width="10" height="10">
											<img src="<?= URL::site('/vendor/boomcms/boom-core/email_assets/spacer.gif', $request) ?>" />
										</td>
										<td width="580" height="10">
											<img src="<?= URL::site('/vendor/boomcms/boom-core/email_assets/spacer.gif', $request) ?>" />
										</td>
										<td width="10" height="10">
											<img src="<?= URL::site('/vendor/boomcms/boom-core/email_assets/spacer.gif', $request) ?>" />
										</td>
									</tr>
								</table>
								<table class="footer" width="100%" cellspacing="0" cellpadding="0" style="margin: 0; padding: 0;background-color:#ffffff;">
									<tr>
										<td width="10" height="30">
											<img src="<?= URL::site('/vendor/boomcms/boom-core/email_assets/spacer.gif', $request) ?>" />
										</td>
										<td width="580" height="30">
											<img src="<?= URL::site('/vendor/boomcms/boom-core/email_assets/spacer.gif', $request) ?>" />
										</td>
										<td width="10" height="30">
											<img src="<?= URL::site('/vendor/boomcms/boom-core/email_assets/spacer.gif', $request) ?>" />
										</td>
									</tr>
									<tr>
										<td width="10" height="10">
											<img src="<?= URL::site('/vendor/boomcms/boom-core/email_assets/spacer.gif', $request) ?>" />
										</td>
										<td width="580" style="font-family: Arial, Helvetica, sans-serif !important; margin: 0; font-size:16px !important;text-align:center;color:#404040;">
											<?= $content ?>
										</td>
										<td width="10" height="10">
											<img src="<?= URL::site('/vendor/boomcms/boom-core/email_assets/spacer.gif', $request) ?>" />
										</td>
									</tr>
									<tr>
										<td width="10" height="30">
											<img src="<?= URL::site('/vendor/boomcms/boom-core/email_assets/spacer.gif', $request) ?>" />
										</td>
										<td width="580" height="30">
											<img src="<?= URL::site('/vendor/boomcms/boom-core/email_assets/spacer.gif', $request) ?>" />
										</td>
										<td width="10" height="30">
											<img src="<?= URL::site('/vendor/boomcms/boom-core/email_assets/spacer.gif', $request) ?>" />
										</td>
									</tr>
								</table>
								<table class="footer" width="100%" cellspacing="0" cellpadding="0" style="margin: 0; padding: 0;background-color:#cccccc;">
									<tr>
										<td width="600" height="10">
											<img src="<?= URL::site('/vendor/boomcms/boom-core/email_assets/spacer.gif', $request) ?>" />
										</td>
									</tr>
									<tr>
										<td style="font-family: Arial, Helvetica, sans-serif !important; margin: 0; font-size:12px !important;text-align:center;color:#404040;">
											<p>You are receiving this email because you are in the project team.</p>
										</td>
									</tr>
									<tr>
										<td width="600" height="10">
											<img src="<?= URL::site('/vendor/boomcms/boom-core/email_assets/spacer.gif', $request) ?>" />
										</td>
									</tr>
								</table>
							</td>
						</tr>
					</table>
				</td>
			</tr>
			<tr>
				<td width="600" height="30">
					<img src="<?= URL::site('/vendor/boomcms/boom-core/email_assets/spacer.gif', $request) ?>" />
				</td>
			</tr>
		</table>
	</body>
</html>
